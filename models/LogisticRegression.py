...
def future_eng(df):
    ...
    er = df['Emulsion_Risk']
    numeric_cols = df.select_dtypes(include=[np.number]).columns.drop('Emulsion_Risk')

    for col1 in numeric_cols:
        df[f'{col1}_dy_dx'] = np.gradient(df[col1].values)
        df[f'{col1}_std_10s'] = df[col1].groupby(pd.Grouper(freq='2s')).transform('std')
        df[f'{col1}_mean_10s'] = df[col1].groupby(pd.Grouper(freq='2s')).transform('mean')
...
# === Model Training ===
model = LogisticRegression(random_state=42,n_jobs=5,max_iter=100000)
